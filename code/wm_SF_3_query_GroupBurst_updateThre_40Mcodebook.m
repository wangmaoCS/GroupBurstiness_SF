...
q_geom  = load_ext([query_dir 'query_feature_geom.float'],5);
...
for k1 = 1:query_num
    ...
    %detect group burstiness
    cur_vw =   q_vw(:, q_cndes(k1)+1:q_cndes(k1+1));
    cur_geom = q_geom(:,q_cndes(k1)+1:q_cndes(k1+1));
    bstGroup   =  wm_DetectBurst(cur_vw,cur_geom,num_voc);
    
    feat_weight = ones(1,n_feat);
    for k2 = 1:length(bstGroup)
       cur_groupNum = length(bstGroup{k2});
       if(cur_groupNum <4)
           continue;
       end
       feat_weight(bstGroup{k2}) = 1 ./ sqrt(cur_groupNum);
       %feat_weight(bstGroup{k2}) = 1 ./ (cur_groupNum);
       %disp(k2);
    end
    ...
    for k2 = 1:knn       
        ...
        % add group burstiness        
        cur_groupWeight = feat_weight(matches(1,:));
        idf_score = idf_score .* cur_groupWeight;
    ...
    save(sprintf('result_updateThre_group_40Mcodebook/matches_burst/matches_%03d.mat',k1),'match_set');
    disp(k1);
end

toc

save(sprintf('result_updateThre_group_40Mcodebook/result_list_top1000_%d_knn5_burst.mat',ht),'result_list');
...
%Elapsed time is 5962.693899 seconds.